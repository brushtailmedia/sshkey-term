...
- **Phase 18: room topics in the terminal app** (display-only). The client has always persisted `RoomInfo.Topic` from the server's `room_list` into the local `rooms` table (since Phase 7b), but nothing ever read it — topics were invisible to users. Phase 18 wires the read path:
  ...
  - **7 new regression tests** covering the store helper (4 cases: normal, missing row, empty topic, update-on-reupsert) and the messages header renderer (7 cases: name+topic shown together, omitted when empty, group context no topic line, DM context no topic line, empty context fallback, SetContext clears topic, RoomTopic accessor round-trip).
  - **Scope: display-only.** Changing a topic after room creation (via a new `sshkey-ctl update-topic` CLI verb) and broadcasting live updates via a new `room_updated` protocol event are deferred to the Admin CLI audit phase — they depend on the broadcast-mechanism decision that phase needs to make once and apply across every state-change CLI command. Today's topic updates are picked up on reconnect when the server re-sends `room_list`. Phase 18 explicitly does not add any server-side code or protocol types.
